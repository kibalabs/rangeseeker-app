...
async def main() -> None:
    ...
    volatilityData = await uniswapClient.get_pool_volatility(POOL_ADDRESS, hoursBack=24)
    volatility = volatilityData.realized
    swapCount = 'aggregated'
    print(f'✓ Current Price: ${currentPrice:,.2f} USDC per WETH')
    print(f'✓ 24h Realized Volatility: {volatility * 100:.2f}%')
    print(f'✓ 24h Annualized Volatility: {volatilityData.annualized * 100:.2f}%')
    print()
